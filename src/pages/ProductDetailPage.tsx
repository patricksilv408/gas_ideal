...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.title} - ${BUSINESS.name} Camaçari | Entrega Rápida`;
      const metaDescription = document.querySelector('meta[name="description"]');
      if (metaDescription) {
        metaDescription.setAttribute('content', `${product.description} Entrega em até ${BUSINESS.deliveryTime} em Camaçari. Peça agora!`);
      }
    }
  }, [product]);
                ...
                <div className="relative">
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span className="font-medium">{BUSINESS.rating}/5 Avaliação</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <div className="flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-3 mb-8 bg-gray-50 p-6 rounded-xl">
                      ...
                      <div className="flex items-center gap-3"><Truck className="w-6 h-6 text-[#00FF00]"/><span className="text-gray-700">Entrega em até {BUSINESS.deliveryTime}</span></div>
                  ...
                  <div>
                    <Button asChild size="lg" disabled={product.availability !== "in stock"} className="w-full bg-[#00FF00] hover:bg-[#00DD00] text-white font-bold text-lg px-8 py-7 rounded-xl shadow-2xl hover:shadow-[#00FF00]/50 transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                      <a href={`https://wa.me/${CONTACT.whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" onClick={() => pushToDataLayer('generate_lead', { conversion_label: 'whatsapp_click', button_location: 'product_section_whatsapp' })} className="flex items-center justify-center gap-3">
